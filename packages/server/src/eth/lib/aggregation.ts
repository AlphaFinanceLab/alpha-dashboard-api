    ...
    for await (const eventsPage of batchedEventsAfterCurrent(paginator)) {
        ...
        for (const ev of eventsPage) {
            ...
            if (ev.contextValues?.bankValues) {
                ...
                GLOBAL_STATE.bankValues.totalETH = convertToBankContractDecimals(
                    new BigNumber(ev.contextValues.bankValues.totalETH)
                );
                GLOBAL_STATE.ETH.lendingValue = convertToBankContractDecimals(
                    new BigNumber(ev.contextValues.bankValues.totalETH)
                );
                // * [PARTIAL] ETH Utilization rate (glbDebtVal / totalETH + reservePool)
                GLOBAL_STATE.ETH.utilizationRate = (
                    GLOBAL_STATE.bankValues.glbDebtVal.dividedBy(GLOBAL_STATE.bankValues.totalETH.plus(GLOBAL_STATE.bankValues.reservePool))
                );
                GLOBAL_STATE.ETH.lendingAPY = borrowInterestRate(GLOBAL_STATE.ETH.utilizationRate)
                    .times(GLOBAL_STATE.ETH.utilizationRate)
